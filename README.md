...
同一个 `main` 分支保存完整产品，不再让组员在多个长期分支之间寻找最新版：

- `web/`：当前正式桌面 Web 产品，包含统一导航、股票研究、决策验证、ETF 诊断、量化研究、交易复盘、全局 AI 助手、可操作新手指引和 90 秒演示；
- `api.py`、`src/`：Python 数据、规则、归因和兼容服务；
- `app.py`、`pages/`：保留的 Streamlit 版本与兼容入口；
- `static/`：Python 服务使用的轻量工具页面；
- `tests/` 与 `web/tests/`：后端与桌面前端的自动化验证。

桌面前端架构、数据边界、AI 降级与已完成/WIP 状态见 [web/TECHNICAL_ARCHITECTURE.md](web/TECHNICAL_ARCHITECTURE.md)。前端本地启动见 [web/README.md](web/README.md)。

版本管理规则：日常改进使用短期功能分支和连续提交，合并后删除分支；只有稳定里程碑才创建 GitHub Release 或版本标签，不把每次界面调整称为一个全新版本。
