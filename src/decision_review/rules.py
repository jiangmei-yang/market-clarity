...
def review_rules(
    ...
    industry_pct = post_industry / capital * 100
    ...
    findings = [
        ...
        finding("industry_limit", "行业仓位上限", industry_pct > profile.max_industry_pct, "计划执行后的行业金额超过你自己设定的边界。", round(industry_pct, 2), profile.max_industry_pct, True),
        ...
        finding("loss_capacity", "可接受亏损", plan.acceptable_loss is None, "尚未说明这笔决定可以承受的金额损失。", "未填写" if plan.acceptable_loss is None else plan.acceptable_loss, profile.max_tolerable_loss),
    ]
    scenarios = [
