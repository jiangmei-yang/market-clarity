...
export default async function EvaluationPage(){
  ...
  return <ProductToolShell active="evaluation" title="质量与验证" description="区分已通过的产品检查、尚未运行的模型评测和真实用户证据。" status="评测结果不使用模拟用户">
    <section className="evaluation-center">
      ...
      <div className="evaluation-gates"><article className="ready"><CheckCircle2/><div><span>确定性规则</span><strong>{baseline.failed?`${baseline.failed} 项需修复`:"20 项基线通过"}</strong><small>社交内容风险与交易前规则</small></div></article><article className={latestModelRun?(latestModelRun.score>=90?"ready":"warning"):generativeReady?"ready":"pending"}>{latestModelRun&&latestModelRun.score>=90?<CheckCircle2/>:<CircleAlert/>}<div><span>真实模型评测</span><strong>{latestModelRun?`${latestModelRun.passed}/${latestModelRun.total} · ${latestModelRun.score} 分`:generativeReady?`${current?.displayName} 已连接，待运行固定任务集`:"未运行"}</strong><small>{latestModelRun?`${latestModelRun.model} · ${formatHongKongDateTime(latestModelRun.runAt)}`:generativeReady?current?.model:"当前只有规则模式；不会用 Mock 冒充模型结果"}</small></div></article><article className="pending"><CircleAlert/><div><span>跨用户证据</span><strong>待真实测试</strong><small>需要任务完成率、行为改变、再次使用和付费意愿</small></div></article></div>
      <EvaluationRunner ready={generativeReady} provider={current?.displayName??"本地规则模式"} model={current?.model??"mock"} initialRun={latestModelRun}/>
      <AgentFunctionalEvaluationRunner ready={generativeReady} provider={current?.displayName??"本地规则模式"} model={current?.model??"mock"} initialRun={latestAgentRun}/>
      ...
      <section className="user-study-summary"><header><div><span>完成后的真实反馈</span><strong>{userStudy.participants} 位匿名参与者 · {userStudy.reviews} 次明确同意的反馈 · {userStudy.representedSegments}/3 类用户</strong></div><a href="/api/evaluation/user-study?format=csv">导出反馈 CSV</a></header><div><article><span>修改或延迟</span><strong>{userStudy.reviews?Math.round(userStudy.changed/userStudy.reviews*100):0}%</strong></article><article><span>自报看懂风险</span><strong>{userStudy.reviews?Math.round(userStudy.understood/userStudy.reviews*100):0}%</strong></article><article><span>提交风险复述</span><strong>{userStudy.reviews?Math.round(userStudy.riskRestatements/userStudy.reviews*100):0}%</strong><small>原文仅供人工核对，不等同于自动判定正确</small></article><article><span>愿意再次使用</span><strong>{userStudy.reviews?Math.round(userStudy.repeatIntent/userStudy.reviews*100):0}%</strong></article><article><span>愿意进入付费测试</span><strong>{userStudy.reviews?Math.round(userStudy.paidIntent/userStudy.reviews*100):0}%</strong></article><article><span>平均完成时间</span><strong>{userStudy.averageSeconds==null?"暂无":`${userStudy.averageSeconds} 秒`}</strong></article></div><p>所有问题默认未选择，只有用户明确作答并同意匿名体验研究后才计入；风险复述需要人工核对是否准确。</p></section>
      <section className="study-cohort-coverage"><header><div><span>招募配额</span><strong>三类目标用户各 5 位</strong></div><small>同一参与者在同一类别只计一次</small></header><div>{["投资经验不足1年","ETF或长期持有","近3个月主动交易"].map(segment=><article key={segment}><span>{segment}</span><strong>{userStudy.segments[segment]??0}<small>/5</small></strong><i><b style={{width:`${Math.min(100,(userStudy.segments[segment]??0)/5*100)}%`}}/></i></article>)}</div></section>
      ...
      <aside className="evaluation-next"><div><strong>下一阶段需要验证什么</strong><p>运行固定模型任务集；邀请目标用户完成同一核心流程；验证持续使用与付费测试意愿。缺少真实证据时，本页会明确显示“尚未验证”。</p></div><span><Link href="/demo">打开 90 秒演示</Link><Link href="/ai-settings">检查模型状态</Link></span></aside>
    </section>
  </ProductToolShell>;
}
