...
export default async function EvaluationPage(){
  ...
  return <ProductToolShell active="evaluation" title="质量与验证" description="区分已通过的产品检查、尚未运行的模型评测和真实用户证据。" status="评测结果不使用模拟用户">
    <section className="evaluation-center">
      ...
      <section className="user-study-summary"><header><div><span>完成后的真实反馈</span><strong>{userStudy.participants} 位匿名参与者 · {userStudy.reviews} 次明确同意的反馈 · {userStudy.representedSegments}/3 类用户</strong></div><a href="/api/evaluation/user-study?format=csv">导出反馈 CSV</a></header><div><article><span>修改或延迟</span><strong>{userStudy.reviews?Math.round(userStudy.changed/userStudy.reviews*100):0}%</strong></article><article><span>自报看懂风险</span><strong>{userStudy.reviews?Math.round(userStudy.understood/userStudy.reviews*100):0}%</strong></article><article><span>提交风险复述</span><strong>{userStudy.reviews?Math.round(userStudy.riskRestatements/userStudy.reviews*100):0}%</strong><small>原文供评委核对，不等同于自动判定正确</small></article><article><span>愿意再次使用</span><strong>{userStudy.reviews?Math.round(userStudy.repeatIntent/userStudy.reviews*100):0}%</strong></article><article><span>愿意进入付费测试</span><strong>{userStudy.reviews?Math.round(userStudy.paidIntent/userStudy.reviews*100):0}%</strong></article><article><span>平均完成时间</span><strong>{userStudy.averageSeconds==null?"暂无":`${userStudy.averageSeconds} 秒`}</strong></article></div><p>所有问题默认未选择，只有用户明确作答并同意课程研究后才计入；风险复述需要人工核对是否准确。</p></section>
