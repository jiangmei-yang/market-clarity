...
type StockFocus = { status:"loading"|"live"|"partial"|"cached"|"unavailable"; provider?:string; dataTimestamp?:string; price?:number; change?:number; points:StockPoint[]; event?:{date:string;title:string;source:string;url?:string}; message?:string };
type Snapshot = {
...
function HomeStockFocus(){
  ...
  const [rangeDays,setRangeDays]=useState<20|60|120>(60);const [chartMode,setChartMode]=useState<"candlestick"|"line">("candlestick");const [chartSize,setChartSize]=useState<"compact"|"standard"|"large">("standard");const [showMa5,setShowMa5]=useState(true);const [showMa20,setShowMa20]=useState(true);const [showVolume,setShowVolume]=useState(true);const [hoverIndex,setHoverIndex]=useState<number|null>(null);const [lockedIndex,setLockedIndex]=useState<number|null>(null);const [chartFullscreen,setChartFullscreen]=useState(false);
