...
type Task={task_id:string;goal:string;status:string;created_at:string;provider:string;model:string;planner_mode:string;reliability?:Reliability;extraction:{user_stage:string;data_requirements:string[];analysis_requirements:string[];ui_requirements:string[];workflow_requirements:string[];missing_information:string[];risk_level:string};steps:Array<{id:string;title:string;tool:string|null;status:string;requires_confirmation:boolean}>;tool_calls:Array<{tool_id:string;status:string;error?:string;output?:unknown;reliability?:Reliability;sources:Array<{name:string;collected_at:string;sample_scope:string;status:string}>}>;workspace_patch?:{summary:string;changes:string[];questions:string[];warnings:string[];affectedModules:string[];preview?:{name?:string}}|null;theme_schema?:{name:string;mode:string;density:string;motion:string;colors:Record<string,string>;accessibility:{high_contrast:boolean;reduced_motion:boolean}};result:unknown;warnings:string[];sources:Array<{source_id:string;name:string;collected_at:string;sample_scope:string;status:string}>;tool_proposal?:{name:string;purpose:string;inputs:string[];outputs:string[];risks:string[]};requires_confirmation:boolean};
...
const statusLabel:Record<string,string>={planning:"规划中",awaiting_confirmation:"待确认",completed:"已完成",failed:"未完成",cancelled:"已取消",pending:"等待",running:"执行中"};
...
export function AgentWorkspace(){
  const [goal,setGoal]=useState("");const [task,setTask]=useState<Task|null>(null);const [history,setHistory]=useState<Task[]>([]);const [providers,setProviders]=useState<Provider[]>([]);const [provider,setProvider]=useState("mock");const [themeId,setThemeId]=useState("light_quiet");const [busy,setBusy]=useState(false);const [error,setError]=useState("");
  ...
  return <div className="agent-os" data-agent-theme={themeId}>
    ...
    {!task?<AgentEmpty/>:<div className="agent-task-grid">
      ...
      {task.workspace_patch&&<section className="agent-patch"><header><Sparkles/><span><strong>工作台修改预览</strong><small>确认前不会改变页面</small></span></header><h2>{task.workspace_patch.summary}</h2><div className="agent-patch-modules">{task.workspace_patch.affectedModules.map((m)=><span key={m}>{moduleNames[m]??m}</span>)}</div>{task.theme_schema&&<div className="agent-theme-preview"><span style={{background:task.theme_schema.colors.accent}}/><div><small>主题预览</small><strong>{task.theme_schema.name} · {task.theme_schema.mode==="dark"?"深色":"浅色"}</strong><p>{task.theme_schema.density} 密度 · {task.theme_schema.motion} 动效{task.theme_schema.accessibility.reduced_motion?" · 减少动态":""}</p></div></div>}<ul>{task.workspace_patch.changes.map((c)=><li key={c}><Check/>{c}</li>)}</ul>{task.workspace_patch.questions.map((q)=><p key={q}><CircleAlert/>{q}</p>)}</section>}
