...
Market Clarity 是个人投资研究与行动前检查工作台；“安心看股”是中文品牌名。产品把股票研究、公开来源核实、交易前决策验证、ETF 持仓诊断、财报体检和交易复盘放在同一条桌面工作流中。

仓库正式名称为 `market-clarity-platform`。日常改进使用连续提交和功能分支；只有可演示、可回滚的稳定里程碑才创建 Release 或版本标签，不把每次界面调整包装成一个全新产品版本。
...
- `/`：个人投资工作台；
- `/analysis?view=research`：股票研究与决策验证；
- `/etf-tool`：ETF 持仓诊断；
- `/trade-tool`：持仓交易复盘。
