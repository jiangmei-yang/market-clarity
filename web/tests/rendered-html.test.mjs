...
test("keeps onboarding contextual and navigation grouped by user goal", async () => {
  ...
  assert.match(workbench, /拖动查看更早或更晚的时间窗口/);
  ...
  assert.match(workbench, /brushRange/);
  assert.match(workbench, /finishBrush/);
  assert.match(workbench, /chart-brush-selection/);
  assert.match(workbench, /横向框选局部放大/);
  assert.match(workbench, /重置缩放/);
  assert.match(workbench, /applyResearchView/);
  assert.match(workbench, /行情与量价/);
  assert.match(workbench, /相对指数/);
  assert.match(workbench, /风险区间/);
  assert.match(workbench, /事件核验/);
  assert.match(workbench, /策略验证/);
...
test("keeps the daily workflow and decision loop in the product source", async () => {
  ...
  assert.match(page, /智能整理并复核/);
  assert.match(page, /本次实时公开资料/);
  assert.match(navigation, /ETF 诊断/);
  assert.match(navigation, /交易前验证/);
  assert.doesNotMatch(navigation, /机会检查/);
  assert.doesNotMatch(navigation, /交易复盘/);
  assert.match(page, /parseReasonStructure/);
  assert.match(page, /系统复核摘要/);
  assert.match(page, /一次完成理由拆解、风险知识检索、公告核实和资产路径/);
  assert.match(page, /确认复核摘要/);
  assert.match(page, /reasonConfirmed/);
  assert.match(page, /确认表示系统理解无误，不表示这些说法真实或未来收益确定/);
  ...
  assert.match(page, /先填写一句理由并完成智能复核/);
  ...
  assert.match(page, /计划后"[\s\S]*?projectedHolding\.toLocaleString\(locale\)/);
