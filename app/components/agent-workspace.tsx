  ...
  return <div className="agent-os" data-agent-theme={themeId}>
    ...
    {!task?<AgentEmpty/>:<div className="agent-task-grid">
      ...
      {task.status==="awaiting_input"&&task.input_request&&<section className="agent-input-request">
        <header><CircleAlert/><span><strong>{task.input_request.title}</strong><small>{task.input_request.description}</small></span><Badge variant="outline">步骤 {task.steps.findIndex((step)=>step.status==="awaiting_input")+1}</Badge></header>
