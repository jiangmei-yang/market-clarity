...
export function EvaluationRunner({ready,provider,model,initialRun}:{ready:boolean;provider:string;model:string;initialRun?:ModelEvaluationRun}){
  ...
  return <section className="model-evaluation-runner"><header><div><span>真实模型固定任务集</span><strong>{provider} · {model||"模型未指定"}</strong></div><button onClick={()=>void execute()} disabled={!ready||busy}>{busy?<RefreshCw className="spin"/>:<Play/>}{busy?"评测中":"运行 20 项评测"}</button></header><p>{message}</p>{run&&<><div className="model-run-summary"><strong>{run.passed}/{run.total}</strong><span>{run.score} 分 · {formatHongKongDateTime(run.runAt)}<small>{run.promptVersion}</small></span></div><div className="model-run-failures">{run.results.filter(item=>!item.passed).length?<>{run.results.filter(item=>!item.passed).map(item=><article key={item.id}><code>{item.id}</code><span><strong>{item.category}</strong><small>{item.answer||"模型未返回该题"}</small></span></article>)}</>:<span>本次固定任务全部通过。该结果不代表所有问题都安全。</span>}</div><details><summary>查看本次原始模型输出</summary><pre>{run.rawOutput}</pre></details></>}</section>;
}
