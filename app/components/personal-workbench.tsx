...
import {
  ...
  ChevronDown, CircleAlert, FileSearch, Gauge, Layers3,
...
export function PersonalWorkbench({ surface, authenticatedUser, initialAIProviders = [] }: { surface: Surface; authenticatedUser: string; initialAIProviders?: AIProviderProfile[] }) {
  ...
  return (
    <div className="personal-shell" data-theme={activeWorkspace.theme.themeId} data-font-scale={activeWorkspace.theme.fontScale} data-radius={activeWorkspace.theme.radius} data-motion={activeWorkspace.theme.motion}>
      ...
      <AppNavigation activePath={surface === "home" ? "/" : `/${surface}`} userName={authenticatedUser} syncLabel={status === "saving" ? "正在保存" : status === "ready" ? "已同步" : status === "loading" ? "正在载入" : "仅本机暂存"} />
      ...
      <nav className="personal-mobile-nav" aria-label="移动端导航"><Link href="/">工作台</Link><Link href="/opportunity">机会检查</Link><Link href="/agent">任务助手</Link><Link href="/profile">我的规则</Link></nav>
    </div>
  );
}
