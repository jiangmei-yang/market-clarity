...
const percentAfter = (text: string, labels: string[], fallback: number) => {
  for (const label of labels) {
    const match = text.match(new RegExp(`${label}[^。；，,.;]{0,18}?(?:不超过|上限|最多|控制在|no more than|under|below|limit(?:ed)? to|up to|maximum(?: of)?)?\\s*(\\d+(?:\\.\\d+)?)\\s*%`, "i"));
...
export function parseProfile(text: string): ProfileDraft {
  ...
  if (/(长期|基本面|现金流|利润增长|long[- ]?term|fundamental|cash flow|profit growth)/i.test(source)) strategy = "long_term_fundamental";
  else if (/ETF|指数|index fund/i.test(source)) strategy = "etf_allocation";
  else if (/(波段|均线|技术|swing|moving average|technical)/i.test(source)) strategy = "swing_trading";
  else if (/(主题|赛道|thematic|theme)/i.test(source)) strategy = "thematic";
  const maxSingleWeight = percentAfter(source, ["单一持仓", "单一资产", "单只", "单股", "single (?:position|asset|holding|stock|ETF)"], .3);
  const maxSectorWeight = percentAfter(source, ["行业", "板块", "single (?:sector|industry)", "sector", "industry"], .5);
  const maxDrawdown = percentAfter(source, ["回撤", "最大亏损", "drawdown", "maximum loss", "max loss"], .2);
  const preferredMetrics = [["现金流", "operating_cash_flow"], ["cash flow", "operating_cash_flow"], ["利润增长", "profit_growth"], ["profit growth", "profit_growth"], ["ROE", "roe"], ["估值", "pe"], ["valuation", "pe"], ["股息", "dividend_yield"], ["dividend", "dividend_yield"]]
    .filter(([label]) => source.toLowerCase().includes(label.toLowerCase())).map(([, metric]) => metric);
  const profile: InvestorProfile = { ...DEFAULT_PROFILE, profileId, strategy, preferredMetrics: [...new Set(preferredMetrics)], maxSingleWeight, maxSectorWeight, maxDrawdown, holdingPeriod: /(长期|long[- ]?term|years?)/i.test(source) ? "long_term" : /(短期|short[- ]?term|days?|weeks?)/i.test(source) ? "short_term" : "medium_term" };
  ...
  const assumptions = [];
  const questions = [];
  if (!/(单一持仓|单一资产|单只|单股|single (?:position|asset|holding|stock|ETF))/i.test(source)) { assumptions.push("暂按单一资产上限 30% 生成候选规则"); questions.push("单一资产占比多少时需要提醒？"); }
  if (!/(行业|板块|sector|industry)/i.test(source)) assumptions.push("暂按单一行业上限 50% 生成候选规则");
...
export const WORKSPACE_TEMPLATES: Record<WorkspaceTemplateId, { name: string; strategy: string; density: Density; explanation: Workspace["explanationLevel"]; description: string; modules: ModuleType[]; workflow: WorkspaceWorkflowStep[]; alertFrequency: Workspace["alertFrequency"] }> = {
...
export function analyzeSocialContent(text: string): SocialAnalysis {
  ...
  const emotion = includes(normalized, ["起飞", "翻倍", "最后机会", "错过后悔", "必涨", "闭眼买", "赶紧上车", "take off", "double", "last chance", "guaranteed to rise", "can't lose", "cannot lose", "get in now"]);
  const urgency = includes(normalized, ["今天必须买", "明天就没机会", "最后机会", "最后一班车", "现在不上车就晚了", "马上", "buy today", "too late", "last chance", "final chance", "act now", "right now"]);
  const authority = includes(normalized, ["老师说", "内部消息", "朋友在机构", "大v说", "主力已经进场", "大资金进场", "insider", "inside information", "my friend at", "an expert says", "institutional friend", "smart money"]);
  const profit = includes(normalized, ["收益截图", "收益率", "赚了", "翻倍", "盈利截图", "return screenshot", "profit screenshot", "made money", "gain", "double"]);
  const evidence = includes(normalized, ["公告", "财报", "年报", "数据", "来源", "链接", "估值", "现金流", "filing", "financial report", "annual report", "data", "source", "link", "valuation", "cash flow"]);
  const risk = includes(normalized, ["风险", "回撤", "亏损", "不确定", "止损", "失效条件", "risk", "drawdown", "loss", "uncertain", "stop loss", "invalidation"]);
