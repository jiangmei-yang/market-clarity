...
export type DashboardVisualization="metric"|"table"|"line"|"candlestick"|"gauge"|"matrix"|"list";
export type DashboardModule=WorkspaceModule&{instanceId:string;position:GridPosition;locked:boolean;dataSourceId:string;refreshPolicy:RefreshPolicy;parameters:Record<string,string|number|boolean>;visualization:DashboardVisualization;title?:string};
...
export type DashboardModuleDefinition={moduleId:ModuleType;name:string;version:string;category:"portfolio"|"research"|"risk"|"quant"|"education"|"workflow";description:string;inputSchema:Record<string,string>;outputSchema:Record<string,string>;uiSchema:{minimumWidth:number;minimumHeight:number;defaultWidth:number;defaultHeight:number;defaultVisualization:DashboardVisualization;visualizations:DashboardVisualization[]};dataSources:string[];permissions:string[];refreshOptions:string[];supportsCodeConfig:true;supportsAgentConfig:true};
...
const visualizationOptions=(moduleId:ModuleType,moduleCategory:DashboardModuleDefinition["category"]):DashboardVisualization[]=>{
  if(moduleId==="technical_chart")return ["candlestick","line"];
  if(moduleId.includes("overlap")||moduleId.includes("exposure"))return ["matrix","table","list"];
  if(moduleId.includes("social_heat")||moduleId.includes("social_sentiment"))return ["matrix","gauge","table","line"];
  if(moduleCategory==="risk")return ["gauge","metric","table","line"];
  if(moduleCategory==="quant")return ["line","table","metric","matrix"];
  if(moduleId.includes("financial")||moduleId.includes("valuation"))return ["table","metric","line"];
  return ["metric","list","table"];
};
const defaultVisualization=(options:DashboardVisualization[])=>options[0];
export const DASHBOARD_MODULE_REGISTRY:DashboardModuleDefinition[]=(Object.keys(MODULE_LABELS) as ModuleType[]).map(moduleId=>{const moduleCategory=category(moduleId);const visualizations=visualizationOptions(moduleId,moduleCategory);return {moduleId,name:MODULE_LABELS[moduleId],version:"1.1.0",category:moduleCategory,description:descriptions[moduleCategory],inputSchema:{workspace_id:"string",data_source_id:"string",parameters:"object"},outputSchema:{data_status:"string",updated_at:"string",content:"object"},uiSchema:{minimumWidth:3,minimumHeight:2,defaultWidth:moduleId.includes("overview")||moduleId.includes("backtest")?12:6,defaultHeight:3,defaultVisualization:defaultVisualization(visualizations),visualizations},dataSources:sources(moduleId),permissions:["read_workspace"],refreshOptions:["manual","1h","daily","weekly"],supportsCodeConfig:true,supportsAgentConfig:true};});
...
export function normalizeDashboardWorkspace(input:Workspace|DashboardWorkspace):DashboardWorkspace{
  ...
  const modules=(base.modules??[]).map((item,index)=>{const current=item as Partial<DashboardModule>;const definition=DASHBOARD_MODULE_REGISTRY.find(row=>row.moduleId===item.type)!;const visualization=current.visualization&&definition.uiSchema.visualizations.includes(current.visualization)?current.visualization:definition.uiSchema.defaultVisualization;return {...item,instanceId:current.instanceId??`${item.type}-${index+1}`,position:{x:clamp(current.position?.x??(index%2)*6,0,11),y:Math.max(0,current.position?.y??Math.floor(index/2)*3),w:clamp(current.position?.w??(item.width==="full"?12:item.width==="third"?4:6),definition.uiSchema.minimumWidth,12),h:Math.max(definition.uiSchema.minimumHeight,current.position?.h??3)},locked:current.locked??false,dataSourceId:current.dataSourceId??definition.dataSources[0]??"workspace_state",refreshPolicy:current.refreshPolicy??{mode:"manual",timezone:"Asia/Shanghai"},parameters:current.parameters??{},visualization} satisfies DashboardModule});
...
export type DashboardPatchChange=
  ...
  |{action:"set_visualization";instanceId:string;value:DashboardVisualization}
...
export function validateDashboardPatch(input:unknown,workspace:DashboardWorkspace,allowedSourceIds?:string[]):DashboardPatch{
  ...
  const actions=new Set(["add_module","remove_module","hide_module","show_module","duplicate_module","move_module","resize_module","set_refresh","set_data_source","set_parameters","set_visualization","set_theme","rename_workspace"]);const allowedSources=new Set(allowedSourceIds??DASHBOARD_DATA_SOURCES.map(item=>item.sourceId));
  for(const [index,value] of raw.changes.entries()){if(!value||typeof value!=="object")throw new Error(`changes[${index}] 无效`);const change=value as Record<string,unknown>;if(!actions.has(String(change.action)))throw new Error(`changes[${index}] action 不在白名单`);if(change.action==="add_module"&&!DASHBOARD_MODULE_REGISTRY.some(item=>item.moduleId===change.moduleId))throw new Error(`changes[${index}] 模块未注册`);if("instanceId" in change&&!workspace.modules.some(item=>item.instanceId===change.instanceId))throw new Error(`changes[${index}] 模块实例不存在`);if(change.action==="set_data_source"&&!allowedSources.has(String(change.value)))throw new Error(`changes[${index}] 数据源未授权`);if(change.action==="set_visualization"){const module=workspace.modules.find(item=>item.instanceId===change.instanceId);const definition=module&&DASHBOARD_MODULE_REGISTRY.find(item=>item.moduleId===module.type);if(!definition?.uiSchema.visualizations.includes(change.value as DashboardVisualization))throw new Error(`changes[${index}] 可视化与模块不兼容`);}}
...
export function validateDashboardWorkspace(input:unknown,allowedSourceIds?:string[]):{valid:true;workspace:DashboardWorkspace}|{valid:false;errors:string[]}{
  ...
  const allowed=new Set(allowedSourceIds??DASHBOARD_DATA_SOURCES.map(item=>item.sourceId));const modules=Array.isArray(raw.modules)?raw.modules as Array<Record<string,unknown>>:[];const ids=new Set<string>();for(const [index,item] of modules.entries()){const definition=DASHBOARD_MODULE_REGISTRY.find(row=>row.moduleId===item.type);if(!definition)errors.push(`modules[${index}].type 不是已注册模块`);if(typeof item.instanceId!=="string"||!item.instanceId)errors.push(`modules[${index}].instanceId 不能为空`);else if(ids.has(item.instanceId))errors.push(`instanceId 重复：${item.instanceId}`);else ids.add(item.instanceId);if(!allowed.has(String(item.dataSourceId)))errors.push(`modules[${index}].dataSourceId 未授权`);if(definition&&!definition.uiSchema.visualizations.includes(item.visualization as DashboardVisualization))errors.push(`modules[${index}].visualization 与模块不兼容`);const position=item.position as Record<string,unknown>|undefined;if(!position||!["x","y","w","h"].every(key=>Number.isFinite(position[key])))errors.push(`modules[${index}].position 不完整`);}
...
export function applyDashboardPatch(workspace:DashboardWorkspace,patch:DashboardPatch,allowedSourceIds?:string[]):DashboardWorkspace{
  ...
  const allowed=new Set(allowedSourceIds??DASHBOARD_DATA_SOURCES.map(source=>source.sourceId));for(const change of patch.changes){if(change.action==="add_module"){const def=DASHBOARD_MODULE_REGISTRY.find(item=>item.moduleId===change.moduleId);if(!def)throw new Error("未知模块");const instanceId=`${change.moduleId}-${crypto.randomUUID().slice(0,8)}`;next.modules.push({type:change.moduleId,instanceId,visible:true,order:next.modules.length,width:def.uiSchema.defaultWidth===12?"full":"half",density:next.density,position:{x:change.position?.x??0,y:change.position?.y??next.modules.length*3,w:change.position?.w??def.uiSchema.defaultWidth,h:change.position?.h??def.uiSchema.defaultHeight},locked:false,dataSourceId:def.dataSources[0]??"workspace_state",refreshPolicy:{mode:"manual",timezone:"Asia/Shanghai"},parameters:{},visualization:def.uiSchema.defaultVisualization});continue;}const item="instanceId" in change?next.modules.find(module=>module.instanceId===change.instanceId):undefined;if("instanceId" in change&&!item)throw new Error(`未找到模块实例 ${change.instanceId}`);if(change.action==="remove_module")next.modules=next.modules.filter(module=>module.instanceId!==change.instanceId);else if(change.action==="hide_module")item!.visible=false;else if(change.action==="show_module")item!.visible=true;else if(change.action==="duplicate_module"){const copy=structuredClone(item!);copy.instanceId=`${copy.type}-${crypto.randomUUID().slice(0,8)}`;copy.position={...copy.position,x:clamp(copy.position.x+1,0,11),y:copy.position.y+1};next.modules.push(copy);}else if(change.action==="move_module"||change.action==="resize_module")item!.position={...item!.position,...change.position};else if(change.action==="set_refresh")item!.refreshPolicy=change.value;else if(change.action==="set_data_source"){if(!allowed.has(change.value))throw new Error("数据源未授权");item!.dataSourceId=change.value;}else if(change.action==="set_parameters")item!.parameters=change.value;else if(change.action==="set_visualization"){const definition=DASHBOARD_MODULE_REGISTRY.find(row=>row.moduleId===item!.type);if(!definition?.uiSchema.visualizations.includes(change.value))throw new Error("可视化与模块不兼容");item!.visualization=change.value;}else if(change.action==="set_theme")next.theme={...next.theme,...change.value};else if(change.action==="rename_workspace")next.name=change.value.trim()||next.name;}
...
export function interpretDashboardInstruction(workspace:DashboardWorkspace,instruction:string):DashboardPatch{
  ...
  for(const [moduleId,label] of mentioned){
    ...
    const visualization=(/K\s*线/.test(scope)?"candlestick":/仪表/.test(scope)?"gauge":/矩阵|热力图/.test(scope)?"matrix":/表格/.test(scope)?"table":/折线|曲线/.test(scope)?"line":/列表/.test(scope)?"list":/数字卡|指标卡/.test(scope)?"metric":undefined) as DashboardVisualization|undefined;
    if(visualization){const definition=DASHBOARD_MODULE_REGISTRY.find(item=>item.moduleId===target.type);if(definition?.uiSchema.visualizations.includes(visualization))unique({action:"set_visualization",instanceId:target.instanceId,value:visualization});}
